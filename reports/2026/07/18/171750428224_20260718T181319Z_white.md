...
- Lichess accuracy: you 88.0%, opponent 98.0%
...
- Your moves: 13 best, 20 excellent, 4 good, 4 inaccuracy, 0 mistake, 1 blunder
...
|---|---|---|---|---|---|
| 14 | -4.0 (piece) | sham | yes | +1.0 pawns | desperado |
| 18 | -5.0 (piece) | sham | yes | +0.0 pawns | puzzle |
| 22 | -9.0 (piece) | real | yes | -9.0 pawns | real_sacrifice_not_gradable |

![Evaluation graph](171750428224_20260718T181319Z_white.png)

## Biggest missed opportunity

You played 23.Qh5. Stockfish preferred Rxf7, after which the main line runs 23. Rxf7 Qb6+ 24. Rf2 Re7 25. Kh1 Rxe5. The evaluation crossed from equal to losing, which matters more than the raw number. Why it went wrong: left pawn on c2, pawn on a2 insufficiently defended. Before committing to a quiet move here, the checklist is checks, captures, threats, in that order. The engine prefers this move from at or below depth 6, which is as shallow as this measurement resolves; it sits near the surface, a forcing move, the kind a checks-and-captures scan catches. Candidates considered by the engine: Rxf7 (-0.96), e6 (-3.34), Qc4 (-4.54).

## Critical positions

- Ply 11 (you), 6.Qxe4: +0.45 -> +0.48 [only-move situation]
- Ply 28 (opponent), 14...Nxf6: -0.15 -> -0.08 [only-move situation]
- Ply 44 (opponent), 22...Nxf7: -0.86 -> -0.96 [only-move situation]
- Ply 45 (you), 23.Qh5: -0.96 -> -4.99 [only-move situation; evaluation crossed equal -> losing]
- Ply 48 (opponent), 24...Qxf7: -6.96 -> -7.28 [only-move situation]
- Ply 50 (opponent), 25...Kxf7: -7.30 -> -7.35 [only-move situation]
- Ply 60 (opponent), 30...Rxh2: -M4 -> -7.65 [forced mate was on the board]

## Your errors, move by move

| Move | Class | Category | WP loss | Refute depth | Seconds spent | Pre-error eval |
|---|---|---|---:|---|---:|---|
| 14.Bxf6 | inaccuracy | attention | 6% | 1 | 130 | balanced |
| 18.Rad1 | inaccuracy | brilliant_sacrifice | 6% | 3 | 48 | balanced |
| 22.Bxf7+ | inaccuracy | attention | 8% | 1 | 7 | balanced |
| 23.Qh5 | blunder | missed_tactic | 28% | 5 | 46 | balanced |
| 24.Qxf7+ | inaccuracy | attention | 6% | 1 | 23 | losing |

### 14.Bxf6 (inaccuracy, positional, wp loss 6%)

You played 14.Bxf6. Stockfish preferred Bh4, after which the main line runs 14. Bh4 Be7 15. Nxf7 Rxf7 16. Bxe6 Nf8. Why it went wrong: left bishop on f6 insufficiently defended. This was a judgment error rather than a missed tactic; compare the pawn structure and piece activity after both moves. The engine prefers this move from at or below depth 6, which is as shallow as this measurement resolves; it sits near the surface, a quiet move, but one whose point shows at a glance. Candidates considered by the engine: Bh4 (+0.50), Nxd7 (+0.21), Bc1 (+0.19).

### 18.Rad1 (inaccuracy, positional, wp loss 6%)

You played 18.Rad1. Stockfish preferred Kh1, after which the main line runs 18. Kh1 Bxe5 19. dxe5 Qxb2 20. Rab1 Qd4. Why it went wrong: left pawn on b2 insufficiently defended. This was a judgment error rather than a missed tactic; compare the pawn structure and piece activity after both moves. The engine does not settle on this move until depth 16; reachable with real calculation, but not on a scan. Candidates considered by the engine: Kh1 (+0.43), c3 (+0.30), Qd3 (+0.10).

### 22.Bxf7+ (inaccuracy, positional, wp loss 8%)

You played 22.Bxf7+. Stockfish preferred h4, after which the main line runs 22. h4 Ne6 23. Qh5 g6 24. Qg4 Qxc2. Why it went wrong: left bishop on f7 insufficiently defended; motif: creates threat on g5. This was a judgment error rather than a missed tactic; compare the pawn structure and piece activity after both moves. The engine prefers this move from at or below depth 6, which is as shallow as this measurement resolves; it sits near the surface, a quiet move, but one whose point shows at a glance. Candidates considered by the engine: h4 (+0.00), Rxf7 (+0.00), Rxg5 (+0.00).

### 23.Qh5 (blunder, tactical, wp loss 28%)

You played 23.Qh5. Stockfish preferred Rxf7, after which the main line runs 23. Rxf7 Qb6+ 24. Rf2 Re7 25. Kh1 Rxe5. The evaluation crossed from equal to losing, which matters more than the raw number. Why it went wrong: left pawn on c2, pawn on a2 insufficiently defended. Before committing to a quiet move here, the checklist is checks, captures, threats, in that order. The engine prefers this move from at or below depth 6, which is as shallow as this measurement resolves; it sits near the surface, a forcing move, the kind a checks-and-captures scan catches. Candidates considered by the engine: Rxf7 (-0.96), e6 (-3.34), Qc4 (-4.54).

### 24.Qxf7+ (inaccuracy, positional, wp loss 6%)

You played 24.Qxf7+. Stockfish preferred Rd7, after which the main line runs 24. Rd7 Nxe5 25. Rxb7 Qa1+ 26. Rf1 Qc3. Why it went wrong: left queen on f7, pawn on c2 insufficiently defended; motif: creates threat on b7. This was a judgment error rather than a missed tactic; compare the pawn structure and piece activity after both moves. The engine prefers this move from at or below depth 6, which is as shallow as this measurement resolves; it sits near the surface, a quiet move, but one whose point shows at a glance. Candidates considered by the engine: Rd7 (-5.01), Re1 (-5.07), h3 (-5.19).
...
- Error categories: 3 attention, 1 brilliant_sacrifice, 1 missed_tactic.
- Attention errors per 100 moves: 7.1.
- Pre-error eval buckets: 0 winning, 4 balanced, 1 losing.
- Middlegame: 5 error(s) (avg wp loss 11%).
